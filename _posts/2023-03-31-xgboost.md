...
```
    ...
    Requirement already satisfied: xgboost in d:\anaconda3\lib\site-packages (1.7.5)
...
#### 데이터 전처리
- 전처리 과정 : fnlwgt, race, capital-gain, capital-loss, native-country 컬럼 삭제
...
xgb_model.fit(X_data_train, y_data_train)
```
...
xgb_result = cross_val_score(xgb_model,X_data_train,y_data_train,cv=5)
xgb_result.mean()
```
...
```python
pre = xgb_model.predict(data_test)
```
